...
func (vsu *VXSocialUpdate) check() error {
	...
	if _, ok := vsu.mutation.UserID(); vsu.mutation.UserCleared() && !ok {
		return errors.New(`ent_work: clearing a required unique edge "VXSocial.user"`)
	}
	return nil
}
...
func (vsuo *VXSocialUpdateOne) sqlSave(ctx context.Context) (_node *VXSocial, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`ent_work: missing "VXSocial.id" for update`)}
	}
