...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withProducts; query != nil {
		if err := oq.loadProducts(ctx, query, nodes, nil,
			func(n *Order, e *Product) { n.Edges.Products = e }); err != nil {
			return nil, err
		}
	}
...
func (oq *OrderQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := oq.ctx.Fields; len(fields) > 0 {
		...
		if oq.withProducts != nil {
			_spec.Node.AddColumnOnce(order.FieldProductsID)
		}
		if oq.withUser != nil {
