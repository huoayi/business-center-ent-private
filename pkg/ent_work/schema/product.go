...
func (Product) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int64("merchant_id").StructTag(`json:"merchant_id"`).Default(0).Comment("外键商户用户 id"),
...
func (Product) Edges() []ent.Edge {
	return []ent.Edge{
		// 逻辑外键
		edge.From("merchant", Merchant.Type).Ref("products").Field("merchant_id").Unique().Required(),
